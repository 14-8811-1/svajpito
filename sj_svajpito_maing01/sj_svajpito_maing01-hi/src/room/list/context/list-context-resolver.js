...
import RoomsLoader from "../rooms-loader";
//@@viewOff:imports
...
const ListContextResolver = createComponent({
  ...
  render({ pageSize, state, children }, ...other) {
    //@@viewOn:hooks
    const roomDataList = useRooms();
    // consume baseUri
    const { data: subAppData } = useSubAppData();

    const shouldLoadData = useMemo(() => checkDataOnContext(roomDataList), [roomDataList]);
    //@@viewOff:hooks
    ...
    if (shouldLoadData) {
      child = (
        <RoomsLoader baseUri={subApp.baseUri} pageSize={pageSize} state={state}>
          {children}
        </RoomsLoader>
      );
    }
